...
public class WEResourceFactory implements IResourceFactory {
	...
	public AMResource getResource(ANode parent, ResourceDescriptor resource, int index) {
		if (resource.getWsType().equals(ResourceDescriptor.TYPE_DATASOURCE_CUSTOM)) {
			...
			if (rp != null && rp.getValue().equals(MRDatasourceWE.CUSTOM_CLASS))
				return new MRDatasourceWE(parent, resource, index);
		}
		return null;
	}
	...
	public ANode createNewDatasource(ANode root, ANode parent) {
		return new MRDatasourceWE(root, MRDatasourceWE.createDescriptor(parent), -1);
	}
